    ...
    this.activatedRoute.data.subscribe(({ commentaire }) => {
      ...
      this.questionService.query().subscribe((res: HttpResponse<IQuestion[]>) => {
        ...
        this.activatedRoute.queryParams.subscribe(data => {
          ...
          const questionId = parseInt(data['question_id']);
          if (questionId) {
            this.editForm.patchValue({ question: this.questions.find(q => q.id === questionId) });
            this.editForm.controls['question'].disable();
          }
        });
      });
    });
